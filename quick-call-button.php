...
Plugin URI: 	https://codez.vn/plugins/
Description: 	add quick call button, call now button for any wordpress website on ipad and mobile phone.
Tags: 			Codez, quick call button, call now button, ipad, mobile, responsive, buttons, phone, call, contact
Author URI: 	https://codez.vn/
Author: 		Codez
Version: 		1.2.9
License: 		GPL2
...
define('QUICK_CALL_BUTTON_VERSION', '1.2.9');
...
class LV_Quick_Call_Button
{
	...
	function lv_admin_scripts() {
		...
		if (get_current_screen()->base == 'settings_page_lv_quick_call_button') {
	        wp_register_script( 'lv_js', plugins_url('assets/js/quick-call-button-admin.js', __FILE__), array('jquery'), QUICK_CALL_BUTTON_VERSION, true );
	        wp_enqueue_script( 'lv_js' );
			wp_register_style( 'lv_css', plugins_url('/assets/css/quick-call-button-admin.css', __FILE__) , false, QUICK_CALL_BUTTON_VERSION );
	...
	function lv_options_page() {
		...
		<div class='wrap'>
			...
			<form method="post" action="options.php" class="form-table">
				...
				<table border=0 cellpadding=2 cellspacing="2">
				<tr>
				    <th><?php _e('Top On Mobile','quickcallbutton') ?></th>
				        <td>
				...
				<tr>
				    <th><?php _e('Top On Tablet','quickcallbutton') ?></th>
				        <td>
					        <input name="lv_options[move_topt]" placeholder="50" value='<?php echo $this->lv_options['move_topt']; ?>' /><label>%</label><br />
							<label for="quick-call-button-text-1"><?php _e(' Position On The Top','quickcallbutton') ?></label>
				        </td>
				</tr>
				<tr>
				    <th><?php _e('Top On Desktop','quickcallbutton') ?></th>
				        <td>
					        <input name="lv_options[move_topd]" placeholder="50" value='<?php echo $this->lv_options['move_topd']; ?>' /><label>%</label><br />
							<label for="quick-call-button-text-1"><?php _e(' Position On The Top','quickcallbutton') ?></label>
				        </td>
				</tr>
				...
				<p class="submit">
					...
					<a class="button button-primary" href="https://codez.vn/contact/" target="_blank">Support</a>
				</p>
			</form>
		</div>
		<?php
	...
	function lv_add_buttons() {
		...
		if ($this->lv_mandatory_have_info()) {
			...
			wp_register_style( 'lv_css', QUICK_CALL_BUTTON_URL."assets/css/quick-call-button.css",array(), QUICK_CALL_BUTTON_VERSION );
			wp_enqueue_style( 'lv_css');
			...
			if ( empty( $call_hiden ) ) { 
				...
				$desktop_display = $this->lv_options['call_text_desktop'] ? 'none !important' : 'initial';
				$tablet_display = $this->lv_options['call_text_tablet'] ? 'none !important' : 'initial';
				$mobile_display = $this->lv_options['call_text_mobile'] ? 'none !important' : 'initial';
				...
				<style> 
					...
					.call-now-button .call-text { display: {$desktop_display}; } 
					.quick-call-button { top: {$this->lv_options['move_topd']}%; }
					} 
					@media screen and (max-width: 1024px) and (min-width: 680px) { 
					.call-now-button .call-text { display: {$tablet_display}; } 
					.quick-call-button { top: {$this->lv_options['move_topt']}%; }
					}
					@media screen and (max-width: 680px) { 
					.call-now-button .call-text { display: {$mobile_display}; } 
					.quick-call-button { top: {$this->lv_options['move_top']}%; }
					}
					.quick-call-button { left: {$this->lv_options['move_left']}%; }
					.quick-call-button { background: {$this->lv_options['bg_color']}; }
					.call-now-button a .quick-alo-ph-img-circle, .call-now-button a .quick-alo-phone-img-circle { background-color: {$this->lv_options['call_color']}; }
